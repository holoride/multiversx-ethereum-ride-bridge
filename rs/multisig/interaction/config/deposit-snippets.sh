deployDepositWrapper() {
    CHECK_VARIABLES DEPOSIT_WRAPPER_WASM BRIDGED_TOKENS_WRAPPER SAFE WESDT_SWAP BASE_TOKEN

    mxpy --verbose contract deploy --bytecode=${DEPOSIT_WRAPPER_WASM} --recall-nonce --pem=${ALICE} \
    --gas-limit=100000000 --arguments ${BRIDGED_TOKENS_WRAPPER} ${SAFE} ${WESDT_SWAP} str:${BASE_TOKEN} \
    --metadata-payable \
...
upgradeDepositWrapper() {
    CHECK_VARIABLES DEPOSIT_WRAPPER DEPOSIT_WRAPPER_WASM BRIDGED_TOKENS_WRAPPER WESDT_SWAP BASE_TOKEN CHAIN_SPECIFIC_TOKEN

    mxpy --verbose contract upgrade ${DEPOSIT_WRAPPER} --bytecode=${DEPOSIT_WRAPPER_WASM} --recall-nonce --pem=${ALICE} \
    --gas-limit=100000000 --arguments ${BRIDGED_TOKENS_WRAPPER} ${SAFE} ${WESDT_SWAP} str:${BASE_TOKEN} str:${CHAIN_SPECIFIC_TOKEN} \
    --metadata-payable \
    --send --wait-result --outfile=deploy-deposit-wrapper-testnet.interaction.json --proxy=${PROXY} --chain=${CHAIN_ID} || return
